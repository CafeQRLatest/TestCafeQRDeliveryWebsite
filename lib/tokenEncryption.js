...
// Simple robust obfuscation/reversible encoder compatible with backend AES / URL-Safe Base64
export function encryptOrgId(uuid) {
  if (!uuid) return '';
  try {
    // URL-safe reversible encoding with signature prefix
...
export function decryptOrgId(token) {
  ...
  // If already standard UUID
  if (/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(clean)) {
    return clean;
  }
  // If prefixed with enc_
