  ...
  (error) => {
    ...
    if (status === 401) {
      ...
      if (typeof window !== 'undefined') {
        ...
        if (window.location.pathname !== '/' && !window.location.pathname.startsWith('/order')) {
...
/** GET /delivery/resolve?handle=:handle&branch=:branch */
export const resolveSlug = (handle, branch) =>
  api.get('/delivery/resolve', { params: { handle, branch } });
...
/** POST /delivery/payments/create-order — create Razorpay order for online payment */
export const createDeliveryPaymentOrder = (payload) =>
  api.post('/delivery/payments/create-order', payload);
...
export const getOrderStatus = (orderId, clientId) =>
  api.get(`/delivery/orders/${orderId}`, { params: { clientId } });
...
/** GET /delivery/restaurant/:clientId/reviews — fetch customer reviews & rating */
export const fetchReviews = (clientId, orgId) =>
  api.get(`/delivery/restaurant/${clientId}/reviews`, { params: { orgId } });

/** POST /delivery/restaurant/:clientId/reviews — submit customer review */
export const submitReview = (clientId, payload) =>
  api.post(`/delivery/restaurant/${clientId}/reviews`, payload);
